...
class BybitREST {
  ...
  async getSymbols() {
    ...
    // Implement pagination to retrieve all instruments (avoid single-page truncation).
    try {
      const all = [];
      const limit = 200;
      let page = 1;
      const pageCap = Number.isFinite(Number(PAGINATION_PAGE_CAP)) ? Number(PAGINATION_PAGE_CAP) : 20;
      while (true) {
        const resp = await this.request('GET', '/v5/market/instruments-info', { category: 'linear', page, limit });
        ...
        page += 1;
        if (page > pageCap) {
          debug('Reached PAGINATION_PAGE_CAP for getSymbols, stopping at page', page - 1);
          break;
        }
      }
