...
const defaultSignalsPath = path.resolve(__dirname, '..', 'data', 'signals.json');
...
module.exports = {
  ...
  PAGINATION_PAGE_CAP: Number(process.env.PAGINATION_PAGE_CAP || 20),

  SEED_HISTORICAL: (process.env.SEED_HISTORICAL || 'true') === 'true',
  HIST_LOOKBACK: Number(process.env.HIST_LOOKBACK || 50),
  SEED_SYMBOLS_LIMIT: Number(process.env.SEED_SYMBOLS_LIMIT || 0),
  ...
  // Use environment override if provided, otherwise deterministic absolute path relative to repo
  SIGNALS_FILE: process.env.SIGNALS_FILE ? path.resolve(process.env.SIGNALS_FILE) : defaultSignalsPath
};