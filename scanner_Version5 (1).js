// scanner.js (root creation: duplicates are now overwritten instead of suppressed)
...
class Scanner extends EventEmitter {
  constructor(trader = null) {
    ...
    // Replay persisted roots to any new listener for 'root_signal'
      ...
      if (event === 'root_signal') {
        ...
        setImmediate(() => {
          for (const r of roots) {
            try { this.emit('root_signal', r); } catch (e) { debug('replay root_signal emit error', e && e.message ? e.message : e); }
          }
        });
      }
    });
  ...
  async fetchSymbolsByREST() {
    ...
    try {
      ...
      // Accept multiple shapes; prefer array
  ...
  async handleWSMessage(msg) {
    if (msg.topic && msg.topic.startsWith('kline')) {
      ...
      for (const k of candles) {
        ...
        await this.onCandle(String(symbol).toUpperCase(), String(interval), startSec, open, close, volume);
      }
    ...
    if (isNewCandle) {
      ...
      if (sdata.lastClose !== null) {
        try {
          ...
          debug('macd update(prev close) error', e && e.message ? e.message : e);
        }
        if (macdPrevResult && !sdata.macdReady) {
          sdata.macdReady = true;
          debug('MACD ready for', symbol, tf);
          this.emit('macd_ready', { symbol, tf });
        }
      }
      sdata.lastOpen = open;
  ...
  async checkRootFlip(symbol, tf, macdResult, volume) {
    ...
    if (crossedUp) {
      ...
      // If an existing signal exists at this key, remove its rootIndex mapping so the new signal overwrites it.
      const prev = this.activeRootSignals[key];
      if (prev && prev.id) {
        try { delete this.rootIndex[prev.id]; } catch (e) { /* ignore */ }
