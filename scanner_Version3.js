...
// Normalizes symbol keys to UPPERCASE to avoid mismatches between REST & WS
...
class Scanner extends EventEmitter {
  ...
  async fetchSymbolsByREST() {
    ...
    try {
      ...
      for (const inst of list) {
        let symbol = inst.symbol || inst.name || inst.instId || inst.symbol_name || inst.instrument_name;
        if (!symbol) continue;
  ...
  initSymbol(symbol) {
    if (!symbol) return;
    const sUpper = String(symbol).toUpperCase();
    if (!this.symbolData[sUpper]) {
      this.symbolData[sUpper] = {};
      for (const tf of Array.from(new Set([...ROOT_TFS, ...MTF_TFS]))) {
        this.symbolData[sUpper][tf] = {
          macd: new MACD(),
  ...
  async handleWSMessage(msg) {
    ...
    if (msg.topic && msg.topic.startsWith('kline')) {
      ...
      if (!symbol) {
        debug('handleWSMessage: no symbol parsed from message', msg.topic);
        return;
      }
      symbol = String(symbol).toUpperCase();
  ...
  async onCandle(symbol, tf, startSec, open, close, volume) {
    if (!symbol) return;
    const sUpper = String(symbol).toUpperCase();
    if (!this.symbolData[sUpper] || !this.symbolData[sUpper][tf]) {
      debug('onCandle: symbol/tf not tracked', sUpper, tf);
      return;
    }
    const sdata = this.symbolData[sUpper][tf];
    const isNewCandle = sdata.lastCandleStart !== startSec;
    
    if (isNewCandle) {
      debug('Candle open', sUpper, tf, new Date(startSec * 1000).toISOString(), 'close:', close);
      ...
      if (sdata.lastClose !== null) {
        ...
        if (macdPrevResult && !sdata.macdReady) {
          ...
          debug('MACD ready for', sUpper, tf);
        }
      }
      ...
      if (ROOT_TFS.includes(tf) && sdata.macdReady && macdPrevResult) {
        debug('ROOT TF check triggered', sUpper, tf, 'hist:', macdPrevResult.hist, 'prevHist:', macdPrevResult.prevHist);
        await this.checkRootFlip(sUpper, tf, macdPrevResult, volume);
      } else if (ROOT_TFS.includes(tf)) {
        debug('ROOT TF check skipped', sUpper, tf, 'macdReady:', sdata.macdReady, 'macdPrevResult:', !!macdPrevResult);
      }
      ...
      this.emit('candle_open', { symbol: sUpper, tf, start: startSec, open, close, volume, macdResult: macdPrevResult });
    } else {
  ...
  async checkRootFlip(symbol, tf, macdResult, volume) {
    ...
    // NEW: Detect MACD histogram crossing threshold
    // Flip UP: prevHist < threshold AND hist >= threshold
    const crossedUp = prevHist < threshold && hist >= threshold;
    
    debug('checkRootFlip details', symbol, tf, { 
      prevHist, 
      hist, 
      threshold, 
      crossedUp
  ...
  getSymbolStatus(symbol) {
    ...
    const sUpper = String(symbol).toUpperCase();
    return this.symbolData[sUpper] || null;
  }
