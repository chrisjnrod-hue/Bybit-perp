...
class Alerter {
  ...
  buildMessage() {
    ...
    // per-tf sections, ordered by ROOT_TFS influence (attempt numeric then other)
  ...
  // send an aggregated summary (debounced)
  async sendAggregated() {
    ...
    try {
      ...
      debug('Alerter: failed to send aggregated telegram', err && err.message ? err.message : err);
    }
  }

  // send a concise immediate message for a single root signal
  async sendSingleRoot(sig) {
    if (!sig || !sig.id) return;
    // avoid duplicates (simple guard)
