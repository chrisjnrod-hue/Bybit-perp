...
function createAPI(scanner, trader) {
  ...
  // Now triggers flip detection and returns any created signal (if conditions met)
  app.get('/debug/inject_candle', async (req, res) => {
    try {
      ...
      const sdata = scanner.symbolData[sym] && scanner.symbolData[sym][tf];
      if (!sdata) {
        return res.status(404).json({ error: 'Symbol/TF not tracked', symbol: sym, tf });
      }

      const closeParsed = Number(close);
      const now = Math.floor(Date.now() / 1000);

      // Feed the close into MACD (this mirrors seeding behavior)
      const result = sdata.macd.update(closeParsed);
      sdata.lastClose = closeParsed;
      if (!sdata.lastCandleStart) sdata.lastCandleStart = now;

      // Mark macdReady if primed (prevHist available)
      if (result && result.prevHist !== null && !sdata.macdReady) {
        sdata.macdReady = true;
      }

      // Try to trigger root detection for ROOT_TFS by calling scanner.checkRootFlip
      let createdSignal = null;
      try {
        const ROOT_TFS = require('./config').ROOT_TFS || [];
        if (ROOT_TFS.includes(String(tf)) && sdata.macdReady && result) {
          if (typeof scanner.checkRootFlip === 'function') {
            await scanner.checkRootFlip(sym, String(tf), result, 0);
            const key = `${sym}|${String(tf)}|${sdata.lastCandleStart}`;
            createdSignal = scanner.activeRootSignals[key] || null;
          }
        }
      } catch (e) {
        // non-fatal: keep returning MACD result even if checkRootFlip errors
        // debug is intentionally omitted here to keep the endpoint concise
      }
  ...
  // NEW: Browser-friendly HTML view of signals.json (first N entries) with download link
