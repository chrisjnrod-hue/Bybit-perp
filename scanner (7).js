...
class Scanner extends EventEmitter {
  ...
  async onCandle(symbol, tf, startSec, open, close, volume) {
    ...
    if (isNewCandle) {
      ...
      // Capture previous candle start BEFORE we overwrite it below.
      const prevCandleStart = sdata.lastCandleStart;
      ...
      if (ROOT_TFS.includes(tf) && sdata.macdReady && macdPrevResult) {
        ...
        await this.checkRootFlip(symbol, tf, macdPrevResult, volume, prevCandleStart);
  ...
  // prevCandleStart: the start timestamp (seconds) of the previous candle (if available)
  async checkRootFlip(symbol, tf, macdResult, volume, prevCandleStart = null) {
    if (!macdResult) {
    ...
    if (crossedUp) {
      ...
      // Use the previous candle's start for the signal if available. Falling back to current lastCandleStart if not.
      const signalStart = prevCandleStart || this.symbolData[symbol][tf].lastCandleStart || Math.floor(Date.now() / 1000);
      const key = `${symbol}|${tf}|${signalStart}`;
