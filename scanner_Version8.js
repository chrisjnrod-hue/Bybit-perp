...
class Scanner extends EventEmitter {
  ...
  async onCandle(symbol, tf, startSec, open, close, volume) {
    ...
    if (isNew) {
      ...
      // compute previous-result deterministically: if we have lastClose, feed it to macd to produce prevHist
      ...
      // Only run root flip for configured ROOT_TFS and if macd prev result exists
      if (ROOT_TFS && ROOT_TFS.includes(tf) && sdata.macdReady && macdPrevResult) {
        debug('ROOT TF check triggered', symbol, tf, { prevHist: macdPrevResult.prevHist, hist: macdPrevResult.hist });
        await this._checkRootFlip(symbol, tf, macdPrevResult, volume);
      } else if (ROOT_TFS && ROOT_TFS.includes(tf)) {
        debug('ROOT TF check skipped', symbol, tf, 'macdReady', sdata.macdReady, 'hasPrev', !!macdPrevResult);
        this.emit('root_tf_check_skipped', { symbol, tf, macdReady: sdata.macdReady, hasPrevResult: !!macdPrevResult });
      }

      // feed live open to MACD to keep state consistent
      try {
        const live = sdata.macd.update(open);
        this.emit('macd_live_update', { symbol, tf, input: open, result: live });
      } catch (e) { debug('macd live update error', e && e.message ? e.message : e); }

      this.emit('candle_open', { symbol, tf, start: startSec, open, close, volume, macdResult: macdPrevResult });
    } else {
      // same candle update: just update lastClose and macd
      sdata.lastClose = close;
      try {
