...
// + dedupe logic in updateRootMTF to avoid writing identical mtf entries repeatedly
...
class Scanner extends EventEmitter {
    ...
    if (!this.symbolData[symbol]) {
      ...
      for (const tf of Array.from(new Set([...ROOT_TFS, ...MTF_TFS]))) {
        this.symbolData[symbol][tf] = {
          ...
          lastClose: null,
          alertedRootId: null,
          macdReady: false  // flag: true once MACD has at least 1 prior result used for flip detection
        };
      }
    }
  }
  ...
  // NOTE: avoid writing identical mtf detail repeatedly (debounce/dedupe)
  updateRootMTF(rootId, mtfInfo) {
    ...
    // If the MTF info is identical to what's already set, skip persisting a duplicate mtf record.
    // This reduces the signals.json growth when evaluateAlignment runs frequently.
    try {
      const existing = sig.mtf || null;
      // simple deep-equal via JSON - acceptable here because mtfInfo shape is stable
      const identical = existing && JSON.stringify(existing) === JSON.stringify(mtfInfo);
      if (identical) {
        sig.mtf = mtfInfo; // still update reference/timestamp
        this.emit('root_updated', { rootId, sig });
        return sig;
      }
    } catch (e) {
      debug('mtf compare error', e && e.message ? e.message : e);
    }
