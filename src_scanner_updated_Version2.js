...
// UPDATED: emits 'root_signal' when a root is created so Alerter can pick it up.
...
class Scanner extends EventEmitter {
  ...
  async checkRootFlip(symbol, tf, macdResult, volume) {
    ...
    if (crossedUp) {
      ...
      // IMPORTANT: emit an event so external listeners (Alerter) can act on new root signals
      try {
        this.emit('root_signal', sig);
        debug('Emitted event root_signal for', id, symbol, tf);
      } catch (e) {
