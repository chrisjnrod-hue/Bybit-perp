...
class Scanner extends EventEmitter {
  constructor(trader = null) {
    ...
    // Replay persisted root signals to any newly-registered root_signal listener.
    // This ensures listeners created after scanner.start() (for example alerter in index.js)
    // will still receive the currently active root signals.
    this.on('newListener', (event, listener) => {
      if (event === 'root_signal') {
        const roots = Object.values(this.activeRootSignals);
        if (!roots || roots.length === 0) return;
        // Emit asynchronously so the listener registration completes first
        setImmediate(() => {
          for (const r of roots) {
            try {
              this.emit('root_signal', r);
            } catch (e) {
              debug('replay root_signal emit error', e && e.message ? e.message : e);
            }
          }
        });
      }
