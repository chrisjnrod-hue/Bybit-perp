...
// Now enhanced with REST fallback to fetch symbols and 24h stats, persistent signal storage, and trade invocation.
...
class Scanner extends EventEmitter {
  ...
  async fetchSymbolsByREST() {
    try {
      ...
      // Bybit returns structure: { ret_code, result: { list: [ ... ] } } - attempt to parse
  ...
  async handleInstrumentInfo(payload) {
    // attempt to parse list from payload
  ...
  async checkRootFlip(symbol, tf, macdResult, volume) {
    ...
    if (prevNeg && currPos) {
      // fetch 24h metrics and apply filters
