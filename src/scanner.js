...
// Normalize incoming WS interval strings to canonical TF keys used in config (e.g. "5","15","60","240","D")
function normalizeIntervalToTf(interval) {
  if (!interval) return interval;
  // Accept numbers (strings) unchanged
  if (/^\d+$/.test(String(interval))) return String(interval);

  const s = String(interval).toLowerCase().trim();

  // minutes like "1m", "5m", "15m", "60m"
  const mMatch = s.match(/^(\d+)m$/);
  if (mMatch) {
    return String(Number(mMatch[1])); // "5m" -> "5"
  }

  // hours like "1h", "2h" => convert to minutes ("1h" -> "60")
  ...
  // some providers may provide "1" meaning 1 minute or "1min" - try to handle common variants
  const minMatch = s.match(/^(\d+)(min|minute|minutes)$/);
  if (minMatch) return String(Number(minMatch[1]));

  // fallback: return original string (caller should handle missing key)
  return interval;
}
...
class Scanner extends EventEmitter {
  ...
  async fetchSymbolsByREST() {
    ...
    try {
      ...
      for (const inst of list) {
        ...
        this.initSymbol(symbol.toUpperCase());
      }
  ...
  async handleWSMessage(msg) {
    ...
    if (msg.topic && msg.topic.startsWith('kline')) {
      ...
      // Parse symbol from topic: kline.{interval}.{symbol} (also handle "linear" part)
      ...
      // Normalize TF to canonical key used internally
  ...
  async checkRootFlip(symbol, tf, macdResult, volume) {
    ...
    if (crossedUp) {
      ...
      // Emit event so listeners (alerter, etc.) can react immediately to new root signals
      try {
        this.emit('root_signal', sig);
      } catch (e) {
        debug('emit root_signal error', e && e.message ? e.message : e);
      }
