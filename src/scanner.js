...
// Changes (2026-04-28):
// - Root signals are now created for any MACD crossing on ROOT_TFS (at 5m opens).
// - Filters (24h price/volume, MACD histogram threshold) are evaluated and attached as metadata
//   on the created signal but NO LONGER block signal creation. Filters should be applied later
//   when selecting/placing trades.
// - Added mtfSnapshot to each root signal containing hist/prevHist/macdReady for each MTF_TF so
//   downstream trade selection can run higher-highs MTF checks.
...
class Scanner extends EventEmitter {
  ...
  async onCandle(symbol, tf, startSec, open, close, volume) {
    ...
    if (isNew) {
      ...
      // exactly as during deploy/seeding, then run root checks using the MACD state for each ROOT_TF.
      if (String(tf) === '5') {
  ...
  async _checkRootFlip(symbol, tf, macdResult, volume) {
    ...
    // Fetch metrics but DO NOT use them to block root creation — instead annotate the signal.
    let metrics = null;
    ...
    // Evaluate filters but do NOT block. These will be used later when choosing a trade.
    const pricePass = typeof pct24 === 'number' ? (pct24 >= MIN_24H_PRICE_CHANGE_PCT) : false;
    const volPass = typeof vol24 === 'number' ? (vol24 >= MIN_24H_VOL_CHANGE_PCT) : false;
    const macdHistThresholdPass = (typeof hist === 'number') ? (hist >= threshold) : false;
    ...
    // create/persist root candidate (no filters block creation)
    ...
    const sig = {
      ...
      symbol,
      tf,
      // keep previous strength calc but include raw metrics so trade-selection can refine it
      strength: Math.max(0, hist) + Math.max(0, (pct24 || 0) / 100),
      pct24,
      vol24,
      // raw macd state for root TF
      hist,
      prevHist,
      macd: macdResult.macd,
      signal: macdResult.signal,
      // filter pass/fail flags for downstream use
      filter: {
        pricePass,
        volPass,
        macdHistThresholdPass,
        thresholdUsed: threshold
      },
      // MTF snapshot for downstream higher-highs checking
      mtfSnapshot,
      start: startSec,
    ...
    s.push(sig);
    writeSignals(s);
    info('✓ ROOT SIGNAL CREATED (candidate)', id, symbol, tf, 'startSec', startSec);
    try { this.emit('root_signal', sig); } catch (e) { debug('emit root_signal error', e && e.message ? e.message : e); }
    // emit evaluated metrics so consumers can log/act on filter decisions
    try { this.emit('root_evaluated', { sigKey: key, id, symbol, tf, pct24, vol24, pricePass, volPass, macdHistThresholdPass }); } catch (e) { debug('emit root_evaluated error', e && e.message ? e.message : e); }
