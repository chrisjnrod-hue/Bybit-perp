...
// Updated: improved checkRootFlip flip detection (sign-change + eps + debug logs)
...
class Scanner extends EventEmitter {
  ...
  async checkRootFlip(symbol, tf, macdResult, volume) {
    ...
    if (prevHist === null || typeof prevHist === 'undefined') {
      debug('checkRootFlip: prevHist is null or undefined', symbol, tf, { prevHist, hist });
      this.emit('checkRootFlip_prevHist_null', { symbol, tf, macdResult });
      return;
    }

    // Robust flip detection: prefer sign-change with a small tolerance (eps),
    // but also honor configured threshold for compatibility.
    const cfg = require('./config');
    const threshold = cfg.MACD_HIST_POSITIVE_THRESHOLD ?? 0;
    // allow small epsilon via env override if needed for testing: MACD_EPS
    const eps = Number(process.env.MACD_EPS || 1e-6);

    // Detailed debug info (helps track missed flips in logs)
    debug('checkRootFlip hist check', { symbol, tf, prevHist, hist, threshold, eps });

    let crossedUp = false;

    // If both numeric, check strict sign change (avoids tiny noise)
    if (typeof prevHist === 'number' && typeof hist === 'number') {
      // strong sign-change detection
      if (prevHist < -eps && hist > eps) {
        crossedUp = true;
      } else {
        // compatibility: threshold-based crossing (original behavior)
        if (prevHist < threshold && hist >= threshold) crossedUp = true;
        // fallback: simple sign-change without eps (catch zero-crossings)
    ...
    if (!crossedUp) {
      this.emit('root_no_cross', { symbol, tf, prevHist, hist, threshold, crossedUp });
      return;
    }
