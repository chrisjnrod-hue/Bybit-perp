...
class Scanner extends EventEmitter {
      ...
      if (REST_FALLBACK_IF_WS_FAIL && this.ws) {
        ...
        const handler = (msg) => {
          try {
            ...
            // match configured instrument topic or messages that look like instrument info
            if (msg.topic === topic || msg.topic.startsWith('instrument')) {
              const data = msg.data || msg;
              // common shapes: data.list, data.instruments, data.data, or array
        ...
        // wait briefly to collect any instrument messages
  ...
  async seedHistorical() {
            ...
            try {
              ...
              for (const c of candles) {
                ...
                if (Number.isFinite(close)) {
                  ...
                  if (sdata && sdata.macd) {
                    // Seed MACD and ensure we set lastClose so live logic can use it to compute macdPrevResult
                    sdata.macd.update(close);
                    // set lastClose so the first incoming new candle can use it
                    if (sdata.lastClose !== close) {
                      sdata.lastClose = close;
                      totalClosesSeeded++;
                    }
    ...
    info('Seeding complete at', new Date(this.lastSeedAt).toISOString(), 'candlesProcessed', totalCandlesProcessed, 'lastClosesSet', totalClosesSeeded);

    // Verification: count how many symbols/TFs have a lastClose set (useful to quickly see if seeding primed indicators)
    let primedCount = 0;
    for (const sym of Object.keys(this.symbolData)) {
      for (const tf of Object.keys(this.symbolData[sym])) {
        if (this.symbolData[sym][tf].lastClose !== null) primedCount++;
      }
    }
    info('Seeding verification: primed symbol/TF buckets with lastClose:', primedCount, 'of', Object.keys(this.symbolData).length * tfs.length);

    return { started: true, completedAt: this.lastSeedAt, symbolsSeeded: targetSymbols.length, candlesProcessed: totalCandlesProcessed, lastClosesSet: totalClosesSeeded, primedBuckets: primedCount };
  }
}
