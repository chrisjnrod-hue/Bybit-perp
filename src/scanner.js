// scanner.js (fixed timestamp normalization and TTL expiry)
...
function normalizeStartToSeconds(raw) {
  ...
  const n = Number(raw);
  if (!Number.isNaN(n)) {
    // If very large > 1e12 treat as ms -> convert to seconds
    if (n > 1e12) return Math.floor(n / 1000);
    // If plausible seconds (>= 1e9), keep
...
class Scanner extends EventEmitter {
  ...
  async onCandle(symbol, tf, startSec, open, close, volume) {
    // startSec is epoch seconds (integer)
    ...
    if (isNewCandle) {
      ...
      this.emit('candle_open', { symbol, tf, start: startSec, open, close, volume, macdResult });
    } else {
      sdata.lastClose = close;
  ...
  async checkRootFlip(symbol, tf, macdResult, volume) {
    ...
    if (prevNeg && currPos) {
      ...
      const tfSeconds = this.tfToSeconds(tf); // in seconds
      const sig = {
        id,
        type: 'root',
        symbol,
        ...
        pct24,
        vol24,
        start: startSec, // seconds
        expires: startSec + tfSeconds, // seconds
        status: 'active',
        mtf: null,
        created: Date.now()
      };
      ...
      // schedule expiry using milliseconds
      const expiryMs = (sig.expires * 1000);
      let ttlMs = expiryMs - Date.now();
      if (ttlMs < 0) ttlMs = 0;
      // setTimeout max safe value ~ 2^31-1; but TTLs here are small (minutes/hours). Guard anyway.
        ...
        try {
          for (const tf of tfs) {
            try {
              ...
              const list = resp?.result?.list || resp?.result || resp?.data || [];
              // Bybit /rest kline items may be in chronological or reverse; ensure oldest-first
              const candles = Array.isArray(list) ? (list.slice().reverse()) : [];
              for (const c of candles) {
                // close might be in different fields
                const close = Number(c.close ?? c.k?.c ?? c.close_price ?? c[4]);
                // start field normalization for historical seeds - some endpoints include 'start' or 't'
                ...
                if (Number.isFinite(close)) {
                  ...
                  // store lastCandleStart if not set (use newest in this loop)
