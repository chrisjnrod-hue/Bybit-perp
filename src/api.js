...
function createAPI(scanner, trader) {
  ...
  // NEW: View signals.json file contents (Android-friendly GET) - JSON response
  ...
  app.get('/debug/view_signals', (req, res) => {
    try {
      ...
      const signals = readSignals() || [];
      const limit = Math.max(1, Math.min(2000, Number(req.query.limit || 500)));
      const slice = signals.slice(0, limit);
      const pretty = JSON.stringify(slice, null, 2);
      res.setHeader('Content-Type', 'text/html; charset=utf-8');
      res.send(
        '<!doctype html><html><head><meta charset="utf-8"><title>Signals</title>' +
        '<style>body{font-family:monospace;background:#111;color:#eee;padding:16px}pre{white-space:pre-wrap;word-wrap:break-word}</style>' +
        '</head><body>' +
        `<h1>Signals (${signals.length})</h1>` +
        `<p>showing ${slice.length} entries (limit=${limit})</p>` +
        `<pre>${escapeHtml(pretty)}</pre>` +
        '</body></html>'
      );
    } catch (err) {
      res.status(500).send('Error reading signals: ' + (err && err.message ? err.message : String(err)));
    }
  });
