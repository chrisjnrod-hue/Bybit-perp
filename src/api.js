// api.js - debug HTTP endpoints (updated with diagnostic routes + browser view for signals.json)
...
function createAPI(scanner, trader) {
  ...
  // return list of discovered symbols and counts
  ...
  // return MACD state for a given symbol (per TF)
  ...
  // Comprehensive root TF debug endpoint
  ...
  // Inject candle via query params
  ...
  // View signals.json file contents (Android-friendly GET)
  ...
  // NEW: Browser-friendly viewer for signals.json
  app.get('/debug/view_signals', (req, res) => {
    // Simple single-page HTML that fetches /debug/signals_file and renders JSON and a download button.
    res.setHeader('Content-Type', 'text/html; charset=utf-8');
    res.send(`<!doctype html>
<html>
<head>
  <meta charset="utf-8" />
  <title>Signals Viewer</title>
  <style>
    body { font-family: Arial, Helvetica, sans-serif; background:#111; color:#eee; padding: 12px; }
    pre { background:#000; padding:12px; border-radius:6px; overflow:auto; max-height:70vh; }
    .controls { margin-bottom:12px; }
    button { margin-right:8px; padding:8px 12px; }
    input[type=number] { width:80px; margin-right:8px; }
    .meta { color:#9ad; margin-bottom:8px; }
  </style>
</head>
<body>
  <h2>Signals Viewer</h2>
  <div class="controls">
    <label>Limit: <input id="limit" type="number" value="500" min="1" max="2000"></label>
    <button id="refresh">Refresh</button>
    <a id="download" href="/debug/download_signals"><button>Download full signals.json</button></a>
  </div>
  ...
  <script>
    async function load() {
      ...
      try {
        const resp = await fetch('/debug/signals_file?limit=' + limit);
        if (!resp.ok) { document.getElementById('out').textContent = 'HTTP ' + resp.status; return; }
        const data = await resp.json();
        document.getElementById('meta').textContent = 'File: ' + data.filePath + ' | total: ' + data.total + ' | returned: ' + data.returned + (data.truncated ? ' (truncated)' : '');
        document.getElementById('out').textContent = JSON.stringify(data.signals, null, 2);
      } catch (e) {
