...
function createAPI(scanner, trader) {
  ...
  app.get('/debug/symbols', (req, res) => {
    ...
    res.json({ count: arr.length, symbols: arr.slice(0, 500) });
  });
  ...
  // NEW: View signals.json file contents
  app.get('/debug/signals_file', (req, res) => {
    try {
      ...
      res.json({ 
        ...
        signals: signals,
        file_path: require('./config').SIGNALS_FILE
      });
    } catch (err) {
  ...
  // NEW: Download signals.json file
  app.get('/debug/download_signals', (req, res) => {
    try {
      const filePath = require('./config').SIGNALS_FILE;
      if (!fs.existsSync(filePath)) {
        return res.status(404).json({ error: 'signals.json not found' });
      }
  ...
  app.get('/debug/root_tf_analysis', (req, res) => {
    ...
    for (const rootTf of require('./config').ROOT_TFS) {
      ...
      for (const symbol of scanner.symbols) {
        ...
        // Check flip condition (classicFlip OR isAccelerating)
        const threshold = require('./config').MACD_HIST_POSITIVE_THRESHOLD;
        const classicFlip = macd && macd.prevHist !== null && macd.prevHist < 0 && macd.hist >= threshold;
        const prevChange = macd && macd.prevHist !== null ? macd.hist - macd.prevHist : 0;
        const isAccelerating = macd && macd.hist > 0 && macd.prevHist > 0 && prevChange > Math.abs(macd.prevHist) * 0.5;
        
        if (classicFlip || isAccelerating) {
