...
function createAPI(scanner, trader) {
  ...
  // NEW: View signals.json file contents (Android-friendly GET returning JSON)
  ...
  // NEW: Browser-friendly view of signals.json (pretty HTML)
  app.get('/debug/view_signals', (req, res) => {
    try {
      ...
      const pretty = JSON.stringify(subset, null, 2);
      const html = `<!doctype html>
<html>
<head>
  <meta charset="utf-8">
  <title>Signals (${signals.length})</title>
  <style>
    body { font-family: system-ui, -apple-system, "Segoe UI", Roboto, "Helvetica Neue", Arial; background:#0b0b0b; color:#e6e6e6; padding:18px; }
    a { color:#66d9ff }
    pre { background:#0f1720; padding:12px; border-radius:6px; overflow:auto; max-height:80vh; white-space:pre; }
    header { margin-bottom:12px; }
    .meta { margin-bottom:8px; color:#9aa7b2; }
  </style>
</head>
<body>
  <header>
    <h1>Signals (${signals.length})</h1>
    <div class="meta">Showing first ${subset.length} entries. <a href="/debug/download_signals">Download full signals.json</a> | <a href="/debug/signals_file">Raw JSON API</a></div>
  </header>
  <pre>${escapeHtml(pretty)}</pre>
      ...
      res.status(500).send(`<pre>Error reading signals: ${escapeHtml(String(err))}</pre>`);
    }
  });
