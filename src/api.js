...
function createAPI(scanner, trader) {
  ...
  app.get('/debug/root_tf_analysis', (req, res) => {
    ...
    for (const rootTf of require('./config').ROOT_TFS) {
      ...
      for (const symbol of scanner.symbols) {
        ...
        if (crossedUp) tfData.withFlipCondition++;
  ...
  // This version mirrors scanner.onCandle: feed previous close into MACD, use that result for flip detection,
  // then set the new candle state and (optionally) update MACD with the new open to keep live state.
  app.get('/debug/inject_candle', async (req, res) => {
    try {
      const { symbol, tf, close, volume } = req.query;
      ...
      const sdata = scanner.symbolData[sym] && scanner.symbolData[sym][tfStr];
      if (!sdata) {
        return res.status(404).json({ error: 'Symbol/TF not tracked', symbol: sym, tf: tfStr });
      }

      const closeParsed = Number(close);
      const volParsed = Number(volume || 0);
      const now = Math.floor(Date.now() / 1000);

      // 1) Feed PREVIOUS candle close into MACD to produce macdPrevResult (same as onCandle does)
      ...
      if (sdata.lastClose !== null) {
        try {
          ...
          if (macdPrevResult && !sdata.macdReady) {
            ...
            debug('MACD ready (inject) for', sym, tfStr);
          }
        } catch (e) {
      ...
      // 2) Now set new candle state (mimic onCandle)
      // For inject we don't have open value; set lastOpen to previous lastClose or the close
      sdata.lastOpen = (sdata.lastClose !== null) ? sdata.lastClose : closeParsed;
      sdata.lastClose = closeParsed;
      sdata.lastCandleStart = now;

      // 3) If this TF is a ROOT_TF, and macd is primed, attempt flip detection using macdPrevResult
      let createdSignal = null;
      try {
        const ROOT_TFS = require('./config').ROOT_TFS || [];
        if (ROOT_TFS.includes(tfStr) && sdata.macdReady && macdPrevResult) {
          if (typeof scanner.checkRootFlip === 'function') {
            await scanner.checkRootFlip(sym, tfStr, macdPrevResult, volParsed);
            const key = `${sym}|${tfStr}|${sdata.lastCandleStart}`;
            createdSignal = scanner.activeRootSignals[key] || null;
          }
        }
      } catch (e) {
        debug('inject checkRootFlip error', e && e.message ? e.message : e);
      }

      // 4) Optionally update MACD with the new candle's open to keep live state (not used for flip detection)
      try {
        sdata.macd.update(sdata.lastOpen);
      } catch (e) {
        debug('inject macd update(open) error', e && e.message ? e.message : e);
      }
  ...
  // Browser-friendly HTML view of signals.json (first N entries) with download link
  app.get('/debug/view_signals', (req, res) => {
    try {
      ...
      const html = `
        <!doctype html>
        <html>
        <head><meta charset="utf-8"><title>Signals (${signals.length})</title>
          <style>body{font-family:system-ui,Segoe UI,Roboto,Arial;margin:20px} pre{white-space:pre-wrap;word-wrap:break-word;background:#f7f7f7;padding:12px;border-radius:6px}</style>
        </head>
        <body>
          <h1>Signals (${signals.length})</h1>
          <p>Showing ${subset.length} of ${signals.length}. <a href="/debug/download_signals">Download full file</a></p>
          <pre>${escapeHtml(JSON.stringify(subset, null, 2))}</pre>
        </body>
        </html>
      `;
      ...
      res.status(500).send('Error reading signals: ' + String(err));
    }
  });

  // Download the full signals.json file as attachment
