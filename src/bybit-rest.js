...
class BybitREST {
  ...
  async request(method, path, params = {}) {
    ...
    return this.queue.add(async () => {
      try {
        ...
        warn('REST request error', method, url, err && err.message ? err.message : err);
        throw err;
      }
    });
  }
  ...
  async getSymbols() {
    ...
    // Uses CURSOR-BASED pagination (nextPageCursor) per Bybit v5 docs.
    try {
      const all = [];
      const limit = 1000; // max per Bybit v5
      let cursor = '';
      let iterations = 0;
      const maxIterations = 100; // safety cap to prevent runaway loops
      ...
      while (iterations < maxIterations) {
        ...
        debug('getSymbols request', { limit, cursor: cursor ? cursor.substring(0, 20) + '...' : 'none' });
        const resp = await this.request('GET', '/v5/market/instruments-info', params);
        const list = resp?.result?.list || [];
        
        if (!Array.isArray(list) || list.length === 0) {
          debug('getSymbols: empty list received, stopping pagination');
          break;
        }
        
        all.push(...list);
        debug('getSymbols batch received', list.length, 'items, total so far:', all.length);
        
        // Check for nextPageCursor in response
        const nextCursor = resp?.result?.nextPageCursor;
        if (!nextCursor) {
          debug('getSymbols: no nextPageCursor, pagination complete');
          break;
        }
        cursor = nextCursor;
      }
      
      info('getSymbols complete: fetched', all.length, 'instruments in', iterations, 'requests');
      return { result: { list: all } };
    } catch (err) {
      warn('getSymbols error:', err && err.message ? err.message : err);
      throw err;
    }
  }
